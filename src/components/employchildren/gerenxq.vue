...
<script>
import { querygeren, queryemplgr,querysetge ,querybcygxx} from "../../utils/http"
export default {
  data() {
    return {
      form: {
  ...
  methods: {
    // 点击保存更新
    onSubmit() {
			querybcygxx(this.unst).then(res=>{
				console.log(res,'res');
			})
    },
		handlePictureCardPreview() {},
			...
			// 保存个人信息
			onSubmit1() {
				querysetge(this.form).then((res) => {
					console.log(res);
				});
			},
    // 获取个人信息
    getgeren() {
      querygeren(this.$route.query.id).then((res) => {
				this.form=res.data
        // console.log(res, "jiben")
